...
export function RetrospectiveList() {
  const retrospectives = [
    ...
    },
    {
      ...
      emotion: "happy",
      emotionEmoji: "🙂",
      summary: "API 연동 중 CORS 이슈가 발생했지만, 원인을 찾아 해결했습니다. 문제 해결 과정에서 많은 것을 배웠습니다.",
      tags: ["API", "문제해결", "CORS"],
    },
    {
      id: "3",
      ...
      emotion: "tired",
      emotionEmoji: "😩",
      summary: "오늘은 번아웃이 심하게 왔습니다. 무리하지 않고 휴식을 취했고, 내일 다시 시작하기로 했습니다.",
      tags: ["번아웃", "휴식", "자기관리"],
    },
  ]
      ...
      {retrospectives.map((retro) => (
        <Card key={retro.id} className="flex flex-col">
          ...
          <CardContent className="flex-1">
            <p className="text-sm text-muted-foreground line-clamp-3">{retro.summary}</p>
          </CardContent>
          <CardFooter className="flex flex-col items-start gap-3 pt-0">
            <div className="flex flex-wrap gap-1">
              {retro.tags.map((tag) => (
                <Badge key={tag} variant="outline" className="bg-rose-50 dark:bg-rose-950/30">
                  {tag}
                </Badge>
              ))}
            </div>
